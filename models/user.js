...
class User {
  ...
  static async get(username) {
    ...
    user.jobs = appsRes.rows.map(app => app.job_id)
    return user;
  }
  ...
  static async apply(username, jobID) {
    ...
    if (job.rows.length === 0) throw new NotFoundError(`No job with id: ${jobID} found`)
    ...
    if (!result) throw new BadRequestError(`Something went wrong applying`)
    return {username: user.username, job: job.title}
  }
